...
import { ComentariosReceitas } from 'src/app/shared/models/ComentariosReceitas';

@Component({
  selector: 'app-comentarios-receitas',
  templateUrl: './comentarios-receitas.component.html',
  styleUrls: ['./comentarios-receitas.component.css']
})
export class ComentariosReceitasComponent implements OnChanges {
  ...
  getComentarios(idPublicacao){
    this.service.getComentariosReceitas(idPublicacao).subscribe(res =>{
  ...
  onSubmit(){
    ...
    {
      this.aguarda = true;
        this.service.postarComentarioReceita(new ComentariosReceitas(this.cadastro.value)).subscribe(res =>{
          if(res.postado){
