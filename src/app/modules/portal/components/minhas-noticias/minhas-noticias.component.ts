...
@Component({
  ...
  styleUrls: ['./minhas-noticias.component.css']
})
export class MinhasNoticiasComponent implements OnInit {
  ...
  GetPublicacoes(){
    ...
    this.service.getNoticiasByUser(GET_ID).subscribe(res =>{
      this.noticias = res;
  ...
  {
    this.route.navigate(['portal/cadastro-noticias'],{queryParams:{id:id}})
  }

  deletePublicacao(id:string):void{
    this.service.deleteNoticiaById(id).subscribe(res =>{
      if(res)
      {
